...
zync sync from -topic MyTopic -pool PoolA
```
...
```
zync sync to -topic MyTarget -pool PoolA
...
zync sync from -topic MyTopic -pool PoolB -shaper shape.zed
...
`zync` has two sub-commands for synchronizing data:
* `zync sync from` - syncs data from a Kafka topic to a Zed data pool
* `zync sync to` - syncs data from a Zed data pool to a Kafka topic
...
descending order.  `zync` will detect and report an error if syncing
...
all data committed by zync writers has monotonically increasing `kafka.offset`.
...
it is best to configure `zync` with a single writer per pool.
...
> if you run with a single `zync` writer per pool, this will not be a problem.

### Sync To

`zync sync to` formats data from a Zed data pool as Avro and "produces"
...
> Note that `zync sync from` does not currently support multiplexing multiple
